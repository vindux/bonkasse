...
class PrinterService:
    ...
    def configure_printer(self, printer_type: str, interface: str, enabled: bool = True, cash_drawer: bool = True):
        """Configure printer connection settings"""
    ...
    def _initialize_printer(self):
        """Initialize printer connection based on configuration"""
    ...
    def _flush_printer(self):
        """Flush printer buffer (especially important for Windows printers)"""
            ...
            for item in items:
                ...
                if quantity > 1:
                    ...
                    if len(line) > max_name_len:
                        ...
                        self.printer.text(f"{line}{' ' * spaces}{price_str}\n")
    ...
    def print_individual_items(self, items: List[Dict]):
        """Print individual 'bon' for each item in the order"""
        if not self.is_printer_ready():
    ...
    def _print_individual_to_console(self, items: List[Dict]):
        """Fallback console printing for individual items"""
    ...
    def _print_summary_to_console(self, session_sales: Dict[int, Dict], total_revenue: float):
        """Fallback console printing for transaction summary"""
        print(f"\n{'=' * 40}")
